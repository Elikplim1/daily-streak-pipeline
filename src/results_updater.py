...
Finds fixtures in Supabase where:
  ...
  - kickoff_utc < NOW() (should have been played)
  - source_match_id IS NOT NULL (can query API)

Queries API-Football for the actual result and updates the fixture row.
...
def get_stale_fixtures(cursor, limit: int) -> List[dict]:
    """
    Find fixtures past their kickoff that still show NS, oldest first,
